...
class CUI2048:
    ...
    def __init__(self, master):
        ...
        self.master.set_status_bar_text(
            "Use WASD to move the board, Enter to enter the menu, and q to quit"
        )
        ...
        self.score_label = self.master.add_label(
            "Score: 0, Turns: 0", 1, 4, column_span=3
        )
        for i in range(0, 4):
            row = []
            for j in range(0, 4):
        ...
        self.master.add_block_label(
            self.get_logo_text(), 0, 4, row_span=1, column_span=3
        )
        ...
        self.menu.set_focus_text(
            "Use WASD to move the board, arrows to select menu items, and enter to use a menu item"
        )
    ...
    def update_turns_scores(self):
        ...
        self.score_label.set_title(
            "Score: {}, Turn: {}".format(
                self.game_instance.score, self.game_instance.turn
            )
        )
    ...
    def undo_move(self, turn_up_down):
        ...
        if self.undid_move and turn_up_down:
            self.master.show_warning_popup(
                "Warning", "Only one Undo is allowed at a time!"
            )
            return
        elif not self.undid_move and not turn_up_down:
            self.master.show_warning_popup(
                "Warning", "Cannot redo move that has not been undone!"
            )
            return
        if self.prev_board is not None:
            temp_board = copy.deepcopy(self.game_instance.game_board.board_positions)
            self.game_instance.game_board.board_positions = copy.deepcopy(
                self.prev_board
            )
    ...
    def apply_board_state(self):
        ...
        if won:
            self.master.show_yes_no_popup(
                "Congratulations You Won! ! Play Again?", self.play_again
            )
        elif lost:
            self.master.show_yes_no_popup(
                "Game Over, Score: {}. Play Again?".format(self.game_instance.score),
